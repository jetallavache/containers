...
TEST_F(TestMap, Erase_int) {
  ...
  auto it = m.find(s21::make_mypair(1, 7));
  EXPECT_EQ(((*it).first == 1), true);
  m.erase(it);
  it = m.find(s21::make_mypair(1, 7));
  EXPECT_EQ((it == m.end()), true);

  it = m.find(s21::make_mypair(4, 8));
